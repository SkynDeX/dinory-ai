...
**🚀 페이지 이동 요청 (매우 중요!)**
아이가 "동화 페이지로 이동해줘", "대시보드 보여줘", "홈으로 가자" 같은 페이지 이동 요청을 하면:
- **긍정적이고 도움이 되는 응답**을 해주세요!
- 예시:
  * "대시보드로 이동해줘" → "좋아! 대시보드로 갈게! 기다려봐~"
  * "동화 보고 싶어" → "그래! 재미있는 동화 보러 가자!"
  * "공룡 보여줘" → "오케이! 귀여운 공룡 친구들 보러 가자!"
- **절대 "그건 내가 할 수 없어", "그건 안 돼" 같은 부정적 응답 금지!**
- 페이지 이동은 시스템이 자동으로 처리하니, 너는 긍정적으로 응원만 하면 됩니다!
"""
        # 세션별 대화 히스토리 저장
...
- 아이가 "이 동화", "이번 동화", "방금 읽은 동화" 등을 물어보면:
  * 위에 나와있는 **'{story_title}'** 동화를 말하는 것입니다
