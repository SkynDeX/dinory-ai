        ...
        try:
            ...
            response = self.client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "system",
                        "content": "당신은 어린이를 위한 창의적이고 따뜻한 인터랙티브 동화 작가입니다. 아이의 이전 선택을 반영하여 스토리가 자연스럽게 분기되도록 만듭니다. 반드시 순수 한글로만 작성하고, 주인공을 '네가', '너는' 같은 2인칭이 아닌 '작은 토끼가', '꼬마 로봇은' 같은 3인칭 캐릭터 호칭으로 지칭하세요. 각 문장은 줄바꿈으로 구분하여 읽기 쉽게 작성하세요."
                    },
                    {
                        "role":  "user",
                        "content": prompt
